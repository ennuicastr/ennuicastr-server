...
async function cancel(sid) {
    return await nrc.deletePromise("https://api.stripe.com/v1/subscriptions/" + sid, {
...
async function updateSubscription(uid, sid, sconfig) {
    ...
    // Only stripe subscriptions are supported
    var parts = /^stripe:(.*)$/.exec(sid);
    ...
    if (accountCredits.subscription) {
        parts = /^stripe:(.*)$/.exec(accountCredits.subscription_id);
        if (!parts) return fail("Unsupported subscription service");
        prevSubscription = parts[1];
    }

    // Get the subscription details
    var subscription = await nrc.getPromise("https://api.stripe.com/v1/subscriptions/" + sid, {
        headers: { authorization }
    });
    ...
    var level = 0;
    if (priceId === config.stripe.subscription.basic.id)
        level = 1;
    else if (priceId === config.stripe.subscription.hq.id)
        level = 2;
    ...
    let startTime = "0", expiry = "0";
    if (level) {
        startTime = (new Date(subscription.start_date)).toISOString();
        expiry = (new Date(subscription.current_period_end)).toISOString();
    }
    ...
    while (true) {
        try {
            ...
            await db.runP("UPDATE credits SET " +
                "subscription=@LEVEL, " +
                "subscription_expiry=max(datetime('now', '1 day'), datetime(@EXPIRY, '1 day')), " +
...
async function cancelSubscription(uid, sconfig) {
    ...
    if (!result.data || result.data.status !== "canceled")
        return fail("Stripe refused the cancelation?");
