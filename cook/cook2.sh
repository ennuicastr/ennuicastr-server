#!/bin/sh
# Copyright (c) 2017-2023 Yahweasel
#
...
do
    ...
    CAP_FN="$c${TRACK_USER+-}$TRACK_USER.vtt"
...
do
    ...
    if [ "$INCLUDE_CAPTIONS" = "yes" ]
    then
        timeout $DEF_TIMEOUT $NICE "$SCRIPTBASE/vtt.js" $TRACK_STREAMNO < $CAPTION_FILE > "$CAP_FFN" &
    fi
done &
...
if [ "$INCLUDE_INFO" = "yes" ]
then
    if [ "$CONTAINER" = "zip" -o "$CONTAINER" = "aupzip" ]
    then
...
then
    ...
    timeout $DEF_TIMEOUT "$SCRIPTBASE/vtt.js" -u $ID.ogg.users -t < $CAPTION_FILE > $OUTDIR/transcript.txt &
fi
